...
import { toggleShow } from "@/store/dashboardSlice"; // 👈 Import from Redux slice

export default function SideBar() {
  const [openIndex, setOpenIndex] = useState(null);
  const dispatch = useDispatch();

  const show = useSelector((state) => state.dashboard.show);
  ...
  return (
    ...
    >
      {/* Logo */}
      <div
        className={`max-md:flex max-md:justify-between max-md:items-center max-md:px-[20px]`}
      >
      ...
      <ul className="flex flex-col gap-[20px]">
        {items.map((item, index) => (
          ...
          >
            <button
              className={`hover:bg-black/10 flex flex-col gap-[15px] w-full py-[9px] px-[12px]`}
            ...
            >
              <span className="flex justify-between items-center">
                ...
                <span
                  className={`w-[30px] h-[22px] flex justify-center items-center overflow-hidden relative shrink-0
